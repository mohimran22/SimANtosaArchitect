                        ...
                        <tbody>
                            ...
                            @foreach($groupedItems as $floorData)
                                ...
                                <tr class="row-floor">
                                    ...
                                    <td colspan="5">
                                        <strong>
                                            {{ strtoupper($floorData['floor_name']) }}
                                        </strong>
                                    </td>
                                    ...
                                    @foreach($categoryData['items'] as $item)
                                        ...
                                        >
                                            ...
                                            @php
                                                ...
                                                $totalVolRealisasi = (float) $item->weeklyProgresses->sum('volume');

                                                $prestasiKumulatif = $volKontrak > 0
                                                    ? ($totalVolRealisasi / $volKontrak) * 100
                                                    : 0;

                                                $bobotKumulatif =
                                                    $prestasiKumulatif
                                                    * ((float) $item->bobot_percent)
                                                    / 100;

                                            @endphp

                                            <td class="total-pelaksanaan text-end">
                                                {{ number_format(
                                                    $item->weeklyProgresses->sum('volume'),
                                                    3,
                                                    ',',
                                                    '.'
                                                ) }}
                                            ...
                                            <td
                                                class="prestasi-kumulatif text-end"
                                                data-item="{{ $item->id }}"
                                            >
                                                {{ number_format($prestasiKumulatif, 2) }}%
                                            </td>

                                            <td
                                                class="bobot-kumulatif text-end"
                                                data-item="{{ $item->id }}"
                                            >
                                                {{ number_format($bobotKumulatif, 3) }}%
                                            </td>
                                        </tr>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function recalcWeek(itemId, weekNo, itemVol, itemBobot)
            {
                ...
                /*
                * Tidak ditampilkan sebagai kolom.
                * Disimpan untuk perhitungan kumulatif.
                */
                el.dataset.progress = progress;
                el.dataset.bobot = bobot;
            }
        ...
        function hitungRealisasiItem(itemId) {
            ...
            const itemVol = parseFloat(row.dataset.itemVol || 0);
            const itemBobot = parseFloat(row.dataset.itemBobot || 0);

            let totalVol = 0;

            row.querySelectorAll('.week-vol').forEach(input => {
                totalVol += parseFloat(input.value || 0);
            });

            const prestasi = itemVol > 0
                ? (totalVol / itemVol) * 100
                : 0;

            const bobot = prestasi * itemBobot / 100;
            const totalVolEl = row.querySelector('.total-pelaksanaan');

            if (totalVolEl) {
                totalVolEl.innerText = totalVol.toLocaleString('id-ID', {
                    minimumFractionDigits: 3,
                    maximumFractionDigits: 3
                });
            }

            const prestasiEl = row.querySelector('.prestasi-kumulatif');

            if (prestasiEl) {
                prestasiEl.innerText =
                    prestasi.toLocaleString('id-ID', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    }) + '%';
            }

            const bobotEl = row.querySelector('.bobot-kumulatif');

            if (bobotEl) {
                bobotEl.innerText =
                    bobot.toLocaleString('id-ID', {
                        minimumFractionDigits: 3,
                        maximumFractionDigits: 3
                    }) + '%';
            }
